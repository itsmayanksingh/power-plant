...
async function create ({ siteId, payload, actor, ipAddress }) {
  ...
  await writeAuditLog({ actorId: actor.id, action: 'parameters.create', module: 'parameters', entityId: row.id, newValues: row, ipAddress })
  return row
}
...
async function update ({ id, payload, actor, ipAddress }) {
  ...
  await writeAuditLog({ actorId: actor.id, action: 'parameters.update', module: 'parameters', entityId: id, oldValues: existing, newValues: updates, ipAddress })
  return row
}
...
async function remove ({ id, actor, ipAddress }) {
  ...
  await writeAuditLog({ actorId: actor.id, action: 'parameters.delete', module: 'parameters', entityId: id, ipAddress })
}
...
async function toggle ({ id, actor, ipAddress }) {
  ...
  await writeAuditLog({ actorId: actor.id, action: 'parameters.toggle', module: 'parameters', entityId: id, oldValues: { is_active: parameter.is_active }, newValues: { is_active: row.is_active }, ipAddress })
  return row
}
...
async function reorder ({ siteId, orders, actor, ipAddress }) {
  ...
  await db.transaction(async trx => {
    ...
    await writeAuditLog({ actorId: actor.id, action: 'parameters.reorder', module: 'parameters', entityId: siteId, newValues: { orders }, ipAddress, trx })
  })

  return listBySite({ siteId })
}
